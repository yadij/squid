...
bool
Security::ErrorIsOptional(const char *name)
{
...
using SslErrorShortcuts = std::map<std::string, const Security::ErrorCode *>;

static const SslErrorShortcuts &
TheSslErrorShortcuts()
{
    ...
    // carefully stripping the scope prefix in Security::ParseErrorString().
    static struct SslErrorAlias {
        const char *name;
    ...
    static SslErrorShortcuts *Shortcuts = nullptr;
    if (!Shortcuts) {
        Shortcuts = new SslErrorShortcuts();
        for (int i = 0; TheSslErrorShortcutsArray[i].name; ++i)
            (*Shortcuts)[TheSslErrorShortcutsArray[i].name] = TheSslErrorShortcutsArray[i].errors;
    }
    return *Shortcuts;
}

void
Security::ParseErrorString(const char *name, Security::Errors &errors)
{
    assert(name);

    const auto ssl_error = ErrorCodeFromName(name);
    ...
    const auto it = TheSslErrorShortcuts().find(name);
