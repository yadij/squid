...
Http::Two::FrameParser::parse(const SBuf &aBuf)
{
    ...
    if (parsingStage_ == HTTP_PARSE_FRAMES) {
        ...
        const bool frameMayPad = (sid == Http::FRAME_HEADERS || sid == Http::FRAME_DATA || sid == Http::FRAME_PUSH_PROMISE);
        ...
        const bool frameMayPriority = (sid == Http::FRAME_HEADERS && fh_.flags() == Http2::FLAG_PRIORITY) ||
                                      sid == Http::FRAME_PRIORITY;
        if (frameMayPriority) {
