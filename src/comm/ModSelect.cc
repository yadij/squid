...
// XXX: de-duplicate this mechanism shared with ModPoll
/**
...
#define commCheckUdpIncoming (++statCounter.udp_incoming.ioEevents > (statCounter.udp_incoming.interval>> INCOMING_FACTOR))
...
comm_select_udp_incoming(void)
{
    ...
    if (statCounter.udp_incoming.interval < 0)
        statCounter.udp_incoming.interval = 0;
...
comm_select_tcp_incoming(void)
{
    ...
    if (statCounter.tcp.interval < 0)
        statCounter.tcp.interval = 0;

    if (statCounter.tcp.interval > MAX_INCOMING_INTERVAL)
        statCounter.tcp.interval = MAX_INCOMING_INTERVAL;

    if (nevents > INCOMING_TCP_MAX)
        nevents = INCOMING_TCP_MAX;

    statCounter.tcp.incoming.count(nevents);
}
...
comm_select_dns_incoming(void)
{
    ...
    statCounter.dns.incoming.count(nevents);
}
