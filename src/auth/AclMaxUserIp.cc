...
ACLMaxUserIP::match(Auth::UserRequest::Pointer auth_user_request, Ip::Address const &src_addr)
{
    ...
    auto user = auth_user_request->user();
    assert(user);
    ...
    if (user->ipcount <= static_cast<size_t>(maximum))
        return 0;

    debugs(28, DBG_IMPORTANT, "user '" << auth_user_request->username() << "' tries to use too many IP addresses (max " << maximum << " allowed)!");
    ...
    if (beStrict) {
        ...
        user->removeIp(src_addr);
        debugs(28, 4, "Denying access in strict mode");
    } else {
        /*
         ...
         */
        user->clearIp();
        debugs(28, 4, "Denying access in non-strict mode - flushing the user ip cache");
    }
