...
Auth::Bearer::UserRequest::startHelperLookup(HttpRequest *req, AccessLogEntry::Pointer &al, AUTHCB * handler, void *data)
{
    ...
    if (!static_cast<Auth::Bearer::Config*>(Auth::SchemeConfig::Find("bearer"))->authenticateProgram) {
        debugs(29, DBG_CRITICAL, "ERROR: No Bearer authentication program configured.");
        handler(data);
        return;
    }

    Auth::Bearer::User *bearer_auth = dynamic_cast<Auth::Bearer::User *>(user().getRaw());
    assert(bearer_auth);
    ...
    if (sz<=0) {
        ...
        helperSubmit(bearerauthenticators, buf, Auth::Bearer::UserRequest::HandleReply,
                     new Auth::StateData(this, handler, data));
    }
}

void
Auth::Bearer::UserRequest::authenticate(HttpRequest * aRequest, ConnStateData * conn, Http::HdrType type)
{
    assert(user());
...
Auth::Bearer::UserRequest::HandleReply(void *data, const Helper::Reply &reply)
{
    ...
    case Helper::Okay: {
        ...
        Auth::Bearer::User *usr = dynamic_cast<Auth::Bearer::User *>(auth_user_request->user().getRaw());
    ...
    case Helper::Error: {
        const char *messageNote = reply.notes.findFirst("message");
        const char *ttlNote = reply.notes.findFirst("ttl");
    ...
    case Helper::Unknown:
        debugs(29, DBG_IMPORTANT, "ERROR: Bearer Authentication Helper crashed (" << reply.reservationId << ")");
    ...
    case Helper::BrokenHelper: {
        const char *errNote = reply.notes.findFirst("message");
    ...
    Auth::Bearer::User *local_usr = dynamic_cast<Auth::Bearer::User *>(auth_user_request->user().getRaw());
