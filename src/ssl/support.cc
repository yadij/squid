...
Ssl::Initialize(void)
{
    ...
    if (::Config.SSL.ssl_engine) {
        ...
        if (!ENGINE_set_default(e, ENGINE_METHOD_ALL)) {
            const auto ssl_error = Security::ErrorString(ERR_get_error());
            throw TexcHere(ToSBuf("Failed to initialise SSL engine: ", ssl_error));
        }
    }
#else
    if (::Config.SSL.ssl_engine)
...
Ssl::InitClientContext(Security::ContextPointer &ctx, Security::PeerOptions &peer, long fl)
{
    ...
    if (!peer.sslCipher.isEmpty()) {
        ...
        if (!SSL_CTX_set_cipher_list(ctx.get(), cipher)) {
            const auto ssl_error = Security::ErrorString(ERR_get_error());
            throw TexcHere(ToSBuf("failed to set SSL cipher suite '", cipher, "' : ", ssl_error));
        }
    }
    ...
    if (!peer.certs.empty()) {
        ...
        if (!keys.certFile.isEmpty()) {
            ...
            if (!SSL_CTX_use_certificate_chain_file(ctx.get(), certfile)) {
                const auto ssl_error = Security::ErrorString(ERR_get_error());
                throw TexcHere(ToSBuf("failed to acquire SSL certificate '", certfile, "': ", ssl_error));
            }
            ...
            if (!SSL_CTX_use_PrivateKey_file(ctx.get(), keyfile, SSL_FILETYPE_PEM)) {
                const auto ssl_error = Security::ErrorString(ERR_get_error());
                throw TexcHere(ToSBuf("Failed to acquire SSL private key '", keyfile, "': ", ssl_error));
            }
            ...
            if (!SSL_CTX_check_private_key(ctx.get())) {
                const auto ssl_error = Security::ErrorString(ERR_get_error());
                throw TexcHere(ToSBuf("private key '", certfile, "' does not match public key '", keyfile, "': ", ssl_error));
            }
        }
    }
