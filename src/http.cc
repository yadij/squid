...
{
    if (Config.http.header_forwarded.mode == SquidConfig::Http::ExtForwarded::Mode::fwdDelete)
        return;
    ...
    if (Config.http.header_forwarded.mode == SquidConfig::Http::ExtForwarded::Mode::xffOff) {
        strListAdd(&strFwd, "unknown", ',');
        hdr_out->putStr(Http::HdrType::X_FORWARDED_FOR, strFwd.termedBuf());
        return;
    }

    if (Config.http.header_forwarded.mode == SquidConfig::Http::ExtForwarded::Mode::xffTruncate)
        strFwd = "";
...
copyOneHeaderFromClientsideRequestToUpstreamRequest(const HttpHeaderEntry *e, const String strConnection, const HttpRequest * request, HttpHeader * hdr_out, const int we_do_ranges, const Http::StateFlags &flags)
{
    ...
    case Http::HdrType::FORWARDED:
         ...
         */
        if (Config.http.header_forwarded.mode == SquidConfig::Http::ExtForwarded::Mode::fwdTransparent) {
            hdr_out->addEntry(e->clone());
        }
        break;
