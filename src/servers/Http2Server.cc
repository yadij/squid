...
void
Http::Two::Server::notifyAllContexts(const int)
{
...
Http::Two::Server::applySettings(Http2::SettingsMap &toBeSet, const SBuf &payload, bool doAck)
{
    ...
    const auto *params = reinterpret_cast<const SettingParameter*>(payload.rawContent());
...
Http::Two::Server::processHeadersFrame()
{
    ...
    if (!s) {
        auto x = MasterXaction::MakePortful(port);
...
Http::Two::Server::handleFrame()
{
     ...
     */
    if (!seenFirstSettings && hp_->frameType() != Http::FRAME_SETTINGS)
        return connectionError(Http2::PROTOCOL_ERROR, "Frame #1 was not client SETTINGS");

    debugs(11, 2, "HTTP/2 " << FrameTypeStr(hp_->frameType()) << " received on " << clientConnection);

    switch (hp_->frameType()) {
    case Http::FRAME_DATA:
    ...
    case Http::FRAME_PRIORITY: // ignore for now
        lastUsedStreamId = max(lastUsedStreamId, hp_->frameStreamId());
    ...
    case Http::FRAME_GOAWAY:
        // TODO
        break;

    case Http::FRAME_WINDOW_UPDATE:
        // TODO
        break;

    case Http::FRAME_CONTINUATION:
        lastUsedStreamId = max(lastUsedStreamId, hp_->frameStreamId());
         ...
         * Note: that is 4GB *after* compressing --> approx. 20GB of de-compressed headers with HPACK 80% compression ratio.
         *
    ...
    case Http::FRAME_ALTSVC: // ignore, invalid from client?
        break;

    case Http::FRAME_ORIGIN: // MUST ignore, MUST NOT forward, only valid on origin server connections.
        break;
