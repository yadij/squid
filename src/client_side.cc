...
parseHttpRequest(ConnStateData *csd, const Http1::RequestParserPointer &hp)
{
    ...
    if (isHttp2MagicPrefix) {
        ...
        if (csd->port != NULL && !csd->port->flags.isIntercepted()) {
            hp->parseStatusCode = Http::scMethodNotAllowed;
            return csd->abortRequestParsing("error:http-2.0-not-supported");
        }
...
ConnStateData::afterClientRead()
{
    ...
    if (!clientParseRequests()) {
         ...
         * The ::Server check with connFinishedWithConn() only
