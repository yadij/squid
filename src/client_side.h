...
public:
    ...
    Comm::ConnectionPointer borrowPinnedConnection(const HttpRequestPointer &, const CachePeer *);
    /**
     * Checks if there is pinning info if it is valid. It can close the server side connection
     * if pinned info is not valid.
     \param aRequest   if it is not nil also checks if the pinning info refers to the request client side HttpRequest
     \param aPeer      if it is not nil also check if the CachePeer is the pinning CachePeer
     \return          The details of the server side connection (may be closed if failures were present).
     */
    const Comm::ConnectionPointer validatePinnedConnection(const HttpRequestPointer &aRequest, const CachePeer *aPeer);
    /**
